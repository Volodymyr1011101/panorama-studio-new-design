...
const Hero: NextComponentType<NextPageContext, {}> = () => {
  ...
  const showAdvertising = (
    ...
    bodyLayout: HTMLDivElement | null,
  ) => {
    if (isOpen) {
  ...
  return (
    <>
      {isShow ? (
        ...
        >
          <button
            className={`w-[50px] h-[50px] absolute right-2 top-2`}
            onClick={() =>
              showAdvertising(false, elements.body, elements.bodyBg)
            }
          >
            <svg
          ...
          <h1 className={`text-[#5bc0f0] text-[46px] text-center mb-8`}>
            {t("Promotion.title")}
          </h1>
          <div className={`flex justify-center gap-2 sm:gap-8 mb-4`}>
            ...
            <span
              className={`text-[#fff] text-[50px] font-bold my-auto w-[30px]`}
            >
              +
            </span>
          ...
          <div className={`px-4`}>
            <p
              className={`text-[20px] text-[#fff] lg:text-[34px] tracking-[1px] mb-4`}
            >
      ...
      <div
        className={`${styles.heroText} h-[220px] md:h-[initial]`}
        ref={container}
      >
        {/* <Image {...imageConfig.backgroundMain} className={styles.mainBg} /> */}
        <div className={`${styles.wrapper}`}>
          <h1 className="text-[#5BC0F0]  flex justify-center pt-12 pl-4 sm:pl-0">
            PANORAMA STUDIO
          </h1>
        </div>
        <div className="hidden md:flex align-center justify-end pr-[10%]">
          <p className="text-[white] text-[24px] italic text-6 pl-4 leading-[0.2px]">
            {t("subtitle")}
          </p>
        </div>
