...
import {
  ...
  DEFAULT,
  WHITE,
} from "@/app/[locale]/(pages)/room/[room]/enums";
import { useEffect, useState } from "react";
import {
  equipmentListAquaDark,
  equipmentListAquaLight,
  equipmentListArt,
  equipmentListWhite,
} from "@/app/[locale]/(pages)/room/[room]/data";
...
const PageContent = ({ roomSlug, galleryImages, equipmentImages }: Props) => {
  ...
  return (
    <>
      <div
        className={`pt-[80px] md:pt-0 px-4 relative tracking-[1px] text-[20px]`}
      >
        {load && (
          <div
            className={`bg-white fixed  h-[100vh] left-0 right-0 flex items-center justify-center z-[100]`}
          >
            <PropagateLoader />
          </div>
        )}
        ...
        <div className={`py-8`}>
          <Link
            href={`/reservation`}
            className={`px-6 py-3 text-white bg-[#522c22] w-fit rounded-2xl flex mx-auto hover:scale-[1.05] transition text-[20px] tracking-[1px] ${unbounded.className} font-[300]`}
          >
        ...
        <div
          className={`md:border-t-[1px] border-b-[0px] md:border-b-[1px] border-[black] rounded-l-[19px] flex flex-col md:flex-row mb-8`}
        >
          <div
            className={`flex flex-col items-center w-full md:w-[35%] justify-center`}
          >
          ...
          <div
            className={`m-auto md:w-[66%] flex flex-col items-center leading-[24px] pt-8 md:pt-0`}
          >
            <h2 className={`text-[30px] border-b-[1px] border-[black] mb-4`}>
              {t("equipment")}
            </h2>
            <ul>
              {
